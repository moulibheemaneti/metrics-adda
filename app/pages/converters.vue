<template>
   <main class="page stack stack--loose">
      <ToolGroupSection group="converters" heading="h1" />
      ...
      <section class="stack stack--tight">
         ...
         <CategoryChips mode="hub" exclude="converters" />
      </section>
   </main>
</template>

<script lang="ts" setup>
/// The unit converters hub.
///
...
/// its own sitemap entry, so it can answer the plural query — "unit converter"
...
useAppSeo({
   title: SEO.convertersHub.title,
   description: SEO.convertersHub.description,
})

defineOgImage("Home", {
   title: COPY.groups.converters.heading,
   subtitle: COPY.groups.converters.lede,
})
...
useSchemaOrg([
   ...
   defineItemList({
      itemListElement: toolsByGroup("converters").map((tool) => ({
